export async function processPrimaryTweet(userTargetTweet: any, requestedUserTwitterID: string) {
  return new Promise((resolve, reject) => {
    try {
      ...
      const addBitcoinToTweet: string = tweetText.replace(/#[Bb][iI][tT][cC][oO][Ii][nN]/g, "#Bitcoin *₿*");
      const tweetTextWithoutURL: string = addBitcoinToTweet.replace(/https?:\/\/\S+/g, "");
      ...
      resolve({ tweetID, tweetTextWithoutURL, tweetAuthorId, tweetUserName, tweetName, tweetVerified, tweetProfileImageUrl, tweetMediaFile, tweetUserId, statusURL });
    } catch (err) {
